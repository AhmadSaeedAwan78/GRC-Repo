...
<?php endif; ?>
    ...
    @if (!empty($sub_forms))
      ...
      <td><a class="fs-14" href="{{url('SAR/SubFormAssignees/'.$sub_forms[$i]->id)}}">  Show Internal Users / Assign Forms To Internal Users (<?php echo (isset($sub_forms[$i]->internal_users_count) && !empty($sub_forms[$i]->internal_users_count))?($sub_forms[$i]->internal_users_count):(0); ?>)</a></td>
      <td><a class="fs-14" href="{{url('/SAR/OrgSubFormsList/'.$sub_forms[$i]->id)}}">  Show User Forms / Send Forms To External Users</a></td>
      @endif
          </tr>
      @endfor
    @endif          
