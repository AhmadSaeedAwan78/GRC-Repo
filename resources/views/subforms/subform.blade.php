...
<?php endif; ?>
     <table class="table subformTwo" id="subforms">
        ...
        <?php  endif; ?>
          @if (!empty($sub_forms))
          <tr>
            <th colspan="{{( ( (Auth::user()->role == 2) || (Auth::user()->role == 3) ) || (Auth::user()->user_type == 1) )?(6):(3)}}" style="text-align:center">{{$form_info->title}} Sub Forms</th>  
          </tr>
          @endif
          <tr>
			...
			<th scope="col">Internal Users</th>			
          ...
          <tr>
			...
			<td>
			    <?php
			        ...
			        if ($count>=0):
			    ?>
			        <a class="fs-14" href="{{url('/Forms/OrgSubFormsList/'.$sub_forms[$i]->id.'/?ext_user_only=1')}}"> <span class="adding_circle" style="vertical-align: middle;">{{$count}}</span> Assign To</a>
			    <?php
			        else:
			    ?>
			        <span class="fs-14">0</span> Assign To
			    <?php endif; ?>
			</td>			
			
			<td><a class="fs-14" href="{{url('Forms/SubFormAssignees/'.$sub_forms[$i]->id)}}">  <span class="adding_circle" style="vertical-align: middle;"><?php echo (isset($sub_forms[$i]->internal_users_count) && !empty($sub_forms[$i]->internal_users_count))?($sub_forms[$i]->internal_users_count):(0); ?></span> Assign To</a></td>
