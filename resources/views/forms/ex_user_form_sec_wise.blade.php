	...
	<div class="container-fluid">
		...
		foreach ($questions as $key => $question): 
			...
			<div class="alert alert-success hidden submit-msg" id="submit-msg" style="margin-top:20px;margin-bottom:10px">	 
				<h4>Almost Done!</h4>
				Please review your answers before submitting form and
				click <button class="btn btn-success btn-lg submit">Submit</button> once finalized.
		...
		</div> <!-- end  <div class="margin sec-body" id="section-{{ $section }}-body" num="{{ $section }}" class="sec-heading-detail" style="margin: 20px 30px; display: block;"> -->
			<!-- btn sec -->

		</div> <!-- end <div id="section-n"> -->
		<?php endif; ?>
		...
		<?php if ($display_body_sec_div): 
			...
			<div class="content">
				<p></p>
				<h6 id="{{'ques-'.$question->form_key.'_'.$question->q_id}}">{{ $question->question_num.' '.$question->question }}</h6>
				<script>
			...
			<div id="wrap" class="wrap-content">						
				...
				switch ($type):	
					...
					if (!empty($options)):
						?>
						<select {{ $attr }} class="form form-control" name="{{ $question->form_key.'_'.$question->q_id}}" q-id="{{$question->q_id}}" style="margin-bottom:20px">
							<?php
								...
								foreach ($options as $option):
									...
									if (strtolower($option) == 'date picker option'):											
										...
										<li {{ $attr }} class="es-selectable {{ $selected_class }}" pickr-num-li="{{++$date_picker_count}}" name="li-{{ $question->q_id }}" id="date-picker-li-{{$date_picker_count}}" >
											...
											<input id="date-picker-{{$date_picker_count}}" class="date-picker" pickr-num="{{$date_picker_count}}" name="d-{{ $question->q_id }}" q-id="{{$question->q_id}}" placeholder="Date Picker Option" value="<?php if (isset($filled[$question->form_key]) && !empty($filled[$question->form_key]['additional_resp'])) echo $filled[$question->form_key]['additional_resp']; else 'Date Picker Option'; ?>" type="{{ $type }}">
										</li>
										<?php
									else:
										?>
	                                      <li {{ $attr }} class="es-selectable {{ $selected_class }}" name="{{ $question->form_key.'_'.$question->q_id.(($type=='mc')?('[]'):('')) }}" q-id="{{$question->q_id}}" value="{{ $option }}" type="{{ $type }}" {{($question->question_assoc_type == 2 && $question->form_id == '2')?("assoc=1"):('')}}>{{ $option }}</li>
										<?php
				...
				case('im'):
				?>
				<form {{ $attr }} id="upload_form" enctype="multipart/form-data" method="POST">
					...
					<input {{ $attr }} type="file" name="img" id="file-upload" class="dropify" {{(isset($filled[$question->form_key]['question_response']) && !empty($filled[$question->form_key]['question_response']))?("data-default-file=".URL::to('public/'.$filled[$question->form_key]['question_response'])):('') }}>                                                            
				</form>
				<?php
				break;
				?>
...
<div class="row btn-section">
	...
	<div class="col col-md-6">
		<button class="btn btn-primary btn-lg prev"  sec-num="{{$curr_sec - 1}}" <?php if ($curr_sec - 1 < 1) echo 'disabled'; ?>>< Previous</button>
	</div>
	<div class="col col-md-6 text-right custbtm">
		<button onclick="topFunction()" id="myBtn_top"  class="btn btn-primary btn-lg next"  sec-num="{{$curr_sec + 1}}" <?php if ($curr_sec + 1 > $section) echo 'disabled'; ?>>Next ></button>
	</div>
...
$(".full_asset").hide();
  ...
  //   	var qa_host = document.getElementById('qa_name').value;
  //   	var name_host = document.getElementById('tesst').value;
  //   	var name_host_c = document.getElementById('tesst_c').value;
  //   	let _token   = $('meta[name="csrf-token"]').attr('content');
    	
  //   	if(qa_host != '' && name_host != '' && name_host_c != ''){
  ...
  //   	console.log(data);
	...
	function update_progress_bar (completed_questions, id_num = '', locked = false)
	{
        ...
        //console.log('check if all sections filled '+check_if_all_sections_filled());
