...
class Forms extends Controller
{
	...
	function in_users_show_form ($form_link_id)
    {
      ...
      // get user form questions
        ...
        JOIN	  questions
        ON            form_questions.question_id = questions.id
        LEFT JOIN admin_form_sections  as afs
        ON            questions.question_section_id = afs.id
        LEFT JOIN client_form_sections as cfs
        ON           (cfs.admin_form_sec_id = afs.id AND cfs.client_id = 23)
        WHERE         user_forms.form_link_id = 'ZMBdsfG5YdI3fJuKMH7VIvvuboQITiPL65ahgtmm'
        */
 ...
 */
	...
	public function ajax_lock_user_form (Request $request)
	{
            ...
            {
                $expiration_info = DB::table('sar_client_expiration_settings')->where('client_id', Auth::user()->client_id)->first();
    ...
    public function completed_forms_list ()
    {
        ...
        if (Auth::user()->role == 2 || (Auth::user()->role == 3 && Auth::user()->user_type == 1))
        {
            ...
            */
            ...
            return view('forms.completed_forms_list', compact('completed_forms'));            
        }
	...
	public function subform_assignees ($subform_id)
	{
		...
		$company_users  = DB::table('users')
		                    ...
		                    ->select('users.*', DB::raw('count(users.id) as forms_count'))
		                    ->groupBy('users.id')
							->get()
							->toArray();
		...
		//if ($id != 2)
		{
    		...
    		foreach ($form_info as $key => $questions)
    		{
    		    if (trim($questions->type) == 'cc')
    		    {
                    ...
                    //  echo '<pre>';print_r($questions->question_num);exit;
    		            ...
    		            if ($fkey == 'dd')
    		            {
    		                ...
    		                if (isset($field->data))
    		                {
    		                    if ($field->data == 'assets')
            		            {
                                    ...
                                    $selected = '';
                                    // dd($fill_custom_response->dd);
                                    session()->put('multi_select',$fill_custom_response->dd);
                                    $multi_data=session('multi_select');
                                    // dd($multi_data[0]);
...
/* 		echo "<pre>";
	...
	public function ajax_assign_subform_to_users (Request $req)
	{
		...
		if (isset($user_ids) && !empty($user_ids))
		{
    		...
    		{
    		    if (!DB::table('user_forms')->where('client_id', $client_id)->where('sub_form_id', $subform_id)->where('user_id', $user_id)->exists()) 
    		    {
		...
		{
    		DB::table('user_forms')->where('sub_form_id', $subform_id)->whereIn('user_id', $d_user_ids)->delete();	    
		}		
    ...
    public function store_edit(Request $request, $id)
    {
      $slider = $request['slider'];
            ...
            if ($request->hasFile('images')) 
            {
                ...
                if ( $image_size > 1000000 ) {
                    return redirect('edit_user/'.$id)->with('alert', 'Maximum size of Image 1MB!')->withInput();            
                }
